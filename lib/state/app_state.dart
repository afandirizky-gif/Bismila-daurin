...
class AppState extends ChangeNotifier {
  ...
  List<ScheduledPickup> get pickups => _pickups.where((p) => p.status == 'Terjadwal').toList();
  List<ScheduledPickup> get pastPickups => _pickups.where((p) => p.status == 'Selesai').toList();

  void schedulePickup(DateTime date, String timeSlot, String address, double weight) {
  ...
  List<ChallengeItem> get joinedChallenges => _challenges.where((c) => c.isJoined && !c.isCompleted).toList();
  List<ChallengeItem> get availableChallenges => _challenges.where((c) => !c.isJoined).toList();
  List<ChallengeItem> get completedChallenges => _challenges.where((c) => c.isCompleted).toList();
